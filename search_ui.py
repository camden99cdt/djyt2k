...
class SearchSuggestionsMixin:
    ...
    def on_url_keypress(self: "YTDemucsApp", event):
        if event.keysym in {"Up", "Down", "Return", "Escape"}:
            handled = self.handle_search_navigation(event.keysym)
            if handled:
                return "break"

    def on_url_focus_out(self: "YTDemucsApp", _event):
        self.root.after(150, self.hide_search_dropdown_if_unfocused)

    def hide_search_dropdown_if_unfocused(self: "YTDemucsApp"):
    ...
    def trigger_search(self: "YTDemucsApp", query: str):
        self.search_debounce_id = None
    ...
    def on_search_results(self: "YTDemucsApp", request_id: int, query: str, results: list[SearchResult]):
    ...
    def show_search_dropdown(self: "YTDemucsApp", loading: bool = False):
    ...
    def bind_search_row_click(self: "YTDemucsApp", widget: tk.Widget, index: int):
        widget.bind(
            "<Button-1>",
            lambda _event, i=index: self.apply_search_selection(i),
        )

    def set_highlight(self: "YTDemucsApp", index: int):
    ...
    def handle_search_navigation(self: "YTDemucsApp", keysym: str) -> bool:
    ...
    def apply_search_selection(self: "YTDemucsApp", index: int):
    ...
    def hide_search_dropdown(self: "YTDemucsApp"):
