...
class SearchSuggestionController:
    def __init__(self, root: tk.Misc, url_var: tk.StringVar, url_entry: tk.Widget):
        self.root = root
    ...
    def on_url_text_change(self, *_):
        ...
        self.search_debounce_id = self.root.after(
            1000, lambda t=text: self.trigger_search(t)
        )
    ...
    def trigger_search(self, query: str):
        ...
        def callback(future):
            try:
                ...
                results = []
            self.root.after(
                0, lambda: self.on_search_results(request_id, query, results)
            )
    ...
    def show_search_dropdown(self, loading: bool = False):
        ...
        if loading:
            loading_row = tk.Frame(list_frame, bg="#ffffff", bd=0, relief="flat", padx=4, pady=8)
            ...
            spinner.start(10)
            tk.Label(loading_row, text="Searching...", bg="#ffffff").pack(
                side="left", anchor="w"
            )
        ...
        for idx, result in enumerate(self.search_results):
            ...
            meta_label = tk.Label(
                text_frame, text=meta_text, fg="#666666", bg="#ffffff", anchor="w"
            )
        ...
        row_heights = [
            frame.winfo_height() or frame.winfo_reqheight() for frame in self.search_result_frames
        ]
        if row_heights:
            self.search_row_height_estimate = max(
                self.search_row_height_estimate, max(row_heights)
            )
    ...
    def handle_search_navigation(self, keysym: str) -> bool:
        ...
        if keysym == "Up":
            new_index = (
                len(self.search_result_frames) - 1 if self.highlight_index <= 0 else self.highlight_index - 1
            )
            self.set_highlight(new_index)
            return True
        if keysym == "Return":
